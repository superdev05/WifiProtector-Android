...
import com.wifiprotector.android.VpnTimeUsageTracker;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener {
    ...
    private boolean mNewVpnStarted = false;
    ...
    private VpnTimeUsageTracker timeUsageTracker = new VpnTimeUsageTracker();
    ...
    public void onCreate() {
        ...
        timeUsageTracker.setListener(new VpnTimeUsageTracker.Listener() {
            @Override
            public void onUpdate(long trackedTime, boolean limitExceeded) {
                if (limitExceeded) {
                    showUsageLimitExceededNotification();
                    getManagement().stopVPN();

                    Intent intent = new Intent(VPNApplication.ACTION_LIMIT_EXCEEDED);
                    LocalBroadcastManager.getInstance(OpenVPNService.this).sendBroadcast(intent);
                }
            }
        });
    }
    ...
    private void showNotification(String msg, String tickerText, boolean lowpriority, long when, ConnectionStatus status) {
        ...
        if (status == ConnectionStatus.LEVEL_CONNECTED) {
            nbuilder.setContentText(getString(R.string.vpn_security_enabled));
        }
        ...
        nbuilder.setContentIntent(getConfigureIntent());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent != null ) {
            ...
            if(START_SERVICE.equals(intent.getAction())) return START_NOT_STICKY;
            if(START_SERVICE_STICKY.equals(intent.getAction())) return START_REDELIVER_INTENT;
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level) {
        ...
        if (level.equals(VpnStatus.ConnectionStatus.LEVEL_CONNECTED)) {
            timeUsageTracker.startTracking();
        } else {
            timeUsageTracker.stopTracking();
        }
    }

    private void showUsageLimitExceededNotification() {
        Intent appIntent = new Intent(this, MainActivity.class);
        appIntent.putExtra(MainActivity.EXTRA_FREE_TIME_EXCEEDED, true);
        ...
        NotificationCompat.Style style = new NotificationCompat.BigTextStyle().bigText(getString(R.string.upgrade_to_unlock));
        builder.setContentTitle(getString(R.string.limit_exceeded))
                .setContentText(getString(R.string.upgrade_to_unlock))
        ...
        ((NotificationManager)getSystemService(Context.NOTIFICATION_SERVICE)).notify(VPNApplication.NOTIFICATION_TIME_EXCEEDED_ID, notification);
    }
