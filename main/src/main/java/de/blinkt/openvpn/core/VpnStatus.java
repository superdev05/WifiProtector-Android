...
public class VpnStatus {
	...
	public synchronized static void updateStateString(String state, String msg, int resid, ConnectionStatus level) {
        ...
        broadcastState(state, msg, resid, level);
	}

    private static void broadcastState(String state, String msg, int resid, ConnectionStatus level) {
        Intent intent = new Intent(VPNApplication.ACTION_VPN_STATE_CHANGED);
        intent.putExtra(VPNApplication.EXTRA_VPN_STATE, state);
        intent.putExtra(VPNApplication.EXTRA_VPN_CONNECTION_STATUS, level);

        LocalBroadcastManager.getInstance(VPNApplication.getContext()).sendBroadcast(intent);
    }

    private static void broadcastVpnConnected(String ip) {
        Intent intent = new Intent(VPNApplication.ACTION_VPN_STATE_CHANGED);
        intent.putExtra(VPNApplication.EXTRA_VPN_CONNECTION_STATUS, ConnectionStatus.LEVEL_CONNECTED);
        intent.putExtra(VPNApplication.EXTRA_SERVER_IP, ip);
    ...
    private synchronized static void newLogItem(LogItem logItem) {
        ...
        String log = logItem.getString(VPNApplication.getContext());
        if (log.contains("CONNECTED,SUCCESS")) {
            String logChunks[] = log.split(",");
            final String ip = logChunks[logChunks.length - 1];

            broadcastVpnConnected(ip);
        }
	}
